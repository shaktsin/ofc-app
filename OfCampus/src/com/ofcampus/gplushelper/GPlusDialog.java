...
public class GPlusDialog extends Dialog {
	...
	private final FrameLayout.LayoutParams FILL = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.FILL_PARENT);
	...
	private void setBackground() {
		...
		Drawable bg = getContext().getResources().getDrawable(R.drawable.roundedshapedialogfortwitter);
		...
		String googleAuthorizationRequestUrl = new GoogleAuthorizationRequestUrl(GPlusCredentialStore.CLIENT_ID, GPlusCredentialStore.REDIRECT_URI, GPlusCredentialStore.SCOPE).build();
		mWebView.loadUrl(googleAuthorizationRequestUrl);

		mWebView.setWebViewClient(new GPlusWebClient());
		mWebView.getSettings().setJavaScriptEnabled(true);
		RelativeLayout.LayoutParams FILL_ = new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.FILL_PARENT, RelativeLayout.LayoutParams.FILL_PARENT);
	...
	public class GPlusProfileDataAsync extends AsyncTask<String, Integer, Long> {
		...
		protected Long doInBackground(String... arg0) {
			try {
				accessTokenResponse = new GoogleAuthorizationCodeGrant(new NetHttpTransport(), new JacksonFactory(), GPlusCredentialStore.CLIENT_ID, GPlusCredentialStore.CLIENT_SECRET, code,
						GPlusCredentialStore.REDIRECT_URI).execute();

				JsonFactory jsonFactory = new JacksonFactory();
				HttpTransport transport = new NetHttpTransport();
				GoogleAccessProtectedResource accessProtectedResource = new GoogleAccessProtectedResource(accessTokenResponse.accessToken, transport, jsonFactory, GPlusCredentialStore.CLIENT_ID,
						GPlusCredentialStore.CLIENT_SECRET, accessTokenResponse.refreshToken);
