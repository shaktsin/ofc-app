package com.ofcampus.mediacontroll;
...
public class PhoneMediaControl {
	...
	public static void loadGalleryPhotosAlbums(Context mContext, final int guid) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				Cursor cursor = null;
				try {
					cursor = MediaStore.Images.Media.query(OfCampusApplication.applicationContext.getContentResolver(), MediaStore.Images.Media.EXTERNAL_CONTENT_URI, projectionPhotos, "", null,
	...
	public static void runOnUIThread(Runnable runnable, long delay) {
		if (delay == 0) {
			OfCampusApplication.applicationHandler.post(runnable);
		} else {
			OfCampusApplication.applicationHandler.postDelayed(runnable, delay);
		}
	}

	private static final String[] projectionPhotos = { MediaStore.Images.Media._ID, MediaStore.Images.Media.BUCKET_ID, MediaStore.Images.Media.BUCKET_DISPLAY_NAME, MediaStore.Images.Media.DATA,
